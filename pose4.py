...
# Сохранение информации о координатах запястий каждого человека в JSON файл
json_data = []
frame_index = 0
for frame_people in people_tracks:
    if frame_people:
        frame_data = {
        ...
        for person in frame_people:
            keypoints_wrists = person['keypoints'][5:7]  # Запястья как пятая и шестая ключевые точки
            person_data = {
                'id': person['id'],
                'bbox': person['bbox'],
                'keypoints_wrists': keypoints_wrists
            }
            frame_data['people'].append(person_data)
        json_data.append(frame_data)
        frame_index += 1

# Приведение всех типов данных к стандартным Python типам
for frame in json_data:
    for person in frame['people']:
        person['bbox'] = [int(coord) for coord in person['bbox']]
        person['id'] = int(person['id'])
        person['keypoints_wrists'] = [[float(coord) for coord in wrist] for wrist in person['keypoints_wrists']]

json_filename = f"{save_name}_wrists_coordinates.json"
